  ...
  if (req.headers['content-type'] !== 'application/json') {
    ...
    const siteTitle = pathSegments.reverse().reduce((acc, pathSegment) => {
      if (!pathSegment.startsWith("?") && pathSegment !== req.session.locale && pathSegment !== 'pages') {
        const pathSegementWithoutGermanAutogeneratedSlug = pathSegment.replace(/-2/g, '').replace(/-de/g, '')
        const upperCasePathSegment = pathSegementWithoutGermanAutogeneratedSlug.replace(/-/g, ' ').charAt(0).toUpperCase() + pathSegementWithoutGermanAutogeneratedSlug.replace(/-/g, ' ').substr(1);
        const translagedPathSegement = req.session.locale ? i18n.__(upperCasePathSegment) : upperCasePathSegment
